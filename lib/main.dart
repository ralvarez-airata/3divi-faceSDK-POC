  ...
  Widget getFinalStatus(BuildContext context){
    return new WillPopScope(
        ...
        child: Scaffold(
          body: Stack(
            children: [
              Center(child: Text("Liveness passed: $_livenessPassed")),
              Container(
                height: 150,
                width: 500,
    ...
    return MaterialApp(
      ...
      onGenerateRoute: (settings){
        ...
        if (settings.name == routeHome) {
          ...
          page = VideoProcessing(cameras, _facerecService!, _recognizer!, endRoute, setLivenss, [ActiveLivenessCheckType.TURN_LEFT,ActiveLivenessCheckType.SMILE,ActiveLivenessCheckType.TURN_DOWN], false);
        }
        else if (settings.name == liveness2dRoute){
          page = VideoProcessing(cameras, _facerecService!, _recognizer!, endRoute, setLivenss, [ActiveLivenessCheckType.TURN_LEFT,ActiveLivenessCheckType.SMILE,ActiveLivenessCheckType.TURN_DOWN], true);
        }
        else if (settings.name == smileRoute){
          page = VideoProcessing(cameras, _facerecService!, _recognizer!, endRoute, setLivenss, [ActiveLivenessCheckType.SMILE,], false);
        }
        else if (settings.name == blinkRoute){
          page = VideoProcessing(cameras, _facerecService!, _recognizer!, endRoute, setLivenss, [ActiveLivenessCheckType.BLINK,], false);
        }
